...
const DropDown = ({ selectedValue, onSelectedChange, selectedSub, onSubChange }: Props) => {
    ...
    // ผูกด้วย category_id ตรงๆ (relational) แทน category_idx แบบเดิม
    const subOptions = subcategories.filter(s => s.category_id === selectedValue)
...
<div className="w-full mt-1 mb-1 flex flex-col gap-2">
    <div className="relative w-full">
        ...
        >
            {selectedValue
            ? categories.find(c => c.category_id === selectedValue)?.category_name
            : "เลือกประเภทปัญหา"}
        </button>
        <IoIosArrowForward
            className={`pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[#6B7280] transition-transform ${open ? '-rotate-90' : 'rotate-90'}`}
        ...
        {/* Sub dropdown — แสดงเมื่อเลือก main แล้วเท่านั้น */}
            {selectedValue && (
                <div  key={selectedValue} className="relative w-full mt-5">
                    ...
                    >
                        {selectedSub
                            ? subOptions.find(o => o.subcategory_id === selectedSub)?.subcategory_name
                            : "เลือกประเภทย่อย"
                        }
                    </button>
                    <IoIosArrowForward
