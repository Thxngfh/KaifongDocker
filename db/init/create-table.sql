...
-- SECTION 1: ORGANIZAT,ION & ACCESS CONTROL1
...
CREATE TABLE user_manuals (
    ...
    -- ข้อมูลหลัก
    title          VARCHAR(255) NOT NULL,         -- ชื่อคู่มือ
    description    TEXT,                          -- คำอธิบาย
    manual_type    VARCHAR(20)  NOT NULL,         -- 'OFFICER' | 'CITIZEN'

    -- ไฟล์
    file_type      VARCHAR(20),                   -- 'PDF' | 'IMAGE'
    file_url       TEXT,                          -- URL ไฟล์จริง
    file_size      BIGINT,                        -- ขนาดไฟล์ (bytes)
    thumbnail_url  TEXT,                          -- รูป preview
    image_width    INT,                           -- ความกว้าง (px)
    image_height   INT,                           -- ความสูง (px)
...
CREATE TABLE workflow_logs (
    ...
    CONSTRAINT chk_action_type CHECK (
        action_type IN ('SUBMIT','ASSIGNED','RESOLVE','CLOSE','REJECT','REOPEN','PAUSED')),
...
CREATE TABLE daily_complaint_summary (
    ...
    total_complaints    INT         NOT NULL DEFAULT 0, -- จำนวนรวม
-- แยกตาม status (จากข้อมูลจริง: PENDING/IN_PROGRESS/PAUSED/RESOLVED/CLOSED/REJECTED)
    pending_cases       INT         NOT NULL DEFAULT 0,   -- รอดำเนินการ
    in_progress_cases   INT         NOT NULL DEFAULT 0,   -- กำลังดำเนินการ
    paused_cases        INT         NOT NULL DEFAULT 0,   -- พักงาน
    resolved_cases      INT         NOT NULL DEFAULT 0,   -- แก้ไขแล้ว
    closed_cases        INT         NOT NULL DEFAULT 0,   -- ปิด
    rejected_cases      INT         NOT NULL DEFAULT 0,   -- ถูกปฏิเสธ
-- SLA (จากข้อมูลจริง: breach rate 37.7%)
    sla_breached_cases  INT         NOT NULL DEFAULT 0,
    sla_on_time_cases   INT         NOT NULL DEFAULT 0,
    sla_percentage      NUMERIC(5,2) DEFAULT 0,           -- % สำเร็จตาม SLA
-- เวลาเฉลี่ย (จากข้อมูลจริง: avg = 24.5 ชั่วโมง = 1,470 นาที)
...
CREATE TABLE category_summary (
    ...
    category_name           VARCHAR(255) NOT NULL,   -- เก็บชื่อไว้เลย ไม่ต้อง JOIN ตอน query
    category_code           VARCHAR(50),             -- เช่น INFRA, ENV, HEALTH
    -- จำนวนรวมและแยก status
...
CREATE TABLE IF NOT EXISTS area_summary (
    ...
    tenant_id               UUID        NOT NULL
                                        DEFAULT '00000000-0000-0000-0000-000000000001',
    summary_date            DATE        NOT NULL,
    -- เขตจากข้อมูลจริง (10 เขต ปากเกร็ด)
    district                VARCHAR(100) NOT NULL,
    -- คลองข่อย / คลองพระอุดม / ท่าอิฐ / บางตลาด / บางพลับ
    -- บางพูด / บ้านใหม่ / ปากเกร็ด / อ้อมเกร็ด / เกาะเกร็ด
    province                VARCHAR(100) DEFAULT 'นนทบุรี',
    total_cases             INT         NOT NULL DEFAULT 0,
    pending_cases           INT         NOT NULL DEFAULT 0,
    in_progress_cases       INT         NOT NULL DEFAULT 0,
    resolved_cases          INT         NOT NULL DEFAULT 0,
    ...
    sla_breached_cases      INT         NOT NULL DEFAULT 0,
    sla_percentage          NUMERIC(5,2) DEFAULT 0,
    -- breach_pct ใช้ระบายสี Heatmap (จากข้อมูลจริง: 34–41%)
    breach_percentage       NUMERIC(5,2) DEFAULT 0,
    -- ระดับความเสี่ยง (HIGH/MEDIUM/LOW คำนวณจาก breach_percentage)
...
CREATE TABLE sla_summary (
    ...
    on_time_cases        INTEGER        NOT NULL DEFAULT 0,
    breached_cases       INTEGER        NOT NULL DEFAULT 0,
    avg_response_hours   NUMERIC(8, 1),
    avg_resolution_hours NUMERIC(8, 1),
    sla_percentage       NUMERIC(5, 1),
    -- แยกตาม target
    cases_target_4h      INTEGER        NOT NULL DEFAULT 0,
    cases_target_24h     INTEGER        NOT NULL DEFAULT 0,
    cases_target_3day    INTEGER        NOT NULL DEFAULT 0,
    cases_target_7day    INTEGER        NOT NULL DEFAULT 0,
    -- breach แยกตาม target
    breach_4h            INTEGER        NOT NULL DEFAULT 0,
    breach_24h           INTEGER        NOT NULL DEFAULT 0,
    breach_3day          INTEGER        NOT NULL DEFAULT 0,
    breach_7day          INTEGER        NOT NULL DEFAULT 0,
    -- metadata
...
CREATE INDEX idx_complaints_detail_fts ON complaints USING GIN (to_tsvector('simple', COALESCE(title,'') || ' ' || COALESCE(detail,'')));
-- complaint_files
